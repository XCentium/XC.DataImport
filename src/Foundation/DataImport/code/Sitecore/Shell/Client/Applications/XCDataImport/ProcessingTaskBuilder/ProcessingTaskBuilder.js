define(["sitecore", "backbone"], function (sitecore, backbone) {
  ...
  var Condition = backbone.LayoutManager.extend({
    template: "processingtaskbuilder-condition",
    editRulesButtonSelector: ".sc-processingtask-edit-rules",
    removeConditionsSelector: ".sc-processingtask-remove-condition",
    ...
    ruleChanged: function (ruleId, fireChangeTrigger) {
      ...
      if (fireChangeTrigger) {
          self.parent.model.trigger("sc.listmanagement.processingtaskbuilder.rule.changed");
      }
    }
  });

  sitecore.Factories.createBaseComponent({
    name: "ProcessingTaskBuilder",
    base: "ControlBase",
    selector: ".sc-processingTaskBuilder",
    conditionContainer: ".sc-processingtaskbuilderbody",
    segmentBuilderFrame: ".sc-processingTaskBuilderFrame",
    rulesContainerSelector: "#RulesContainer",
    ruleContainerSelector: ".rule-container",
    segmentBuilderFrameContainer: null,
    attributes: [
    { name: "processingTaskBuilderUrl", value: "$el.data:sc-processingtaskbuilderurl" }
    ],
    initialize: function () {
      this._super();
      var self = this;
      ...
      self.segmentBuilderUrl = self.$el.data("sc-processingtaskbuilderurl") ? self.$el.data("sc-processingtaskbuilderurl") : null;
      var iFrame = $("#ProcessingTaskBuilderFrame");
      if (iFrame) {
        iFrame[0].src = self.segmentBuilderUrl;
        iFrame.load(function () {
          self.segmentBuilderFrameContainer = $("#ProcessingTaskBuilderFrame")[0].contentWindow;
