(function (speak, require) {
    ...
    speak.component(["dynatree", "jQueryPresenter"], {
        ...
        [
          ...
          { name: "database", pluginProperty: "Database", defaultValue: "core" },
          { name: "generateIds", defaultValue: true },
          { name: "cookieId", defaultValue: "dynatree" },
          { name: "idPrefix", defaultValue: "dynatree-id-" }
        ],
        events:
        [
        ...
        initAjax: null,
        initialize: function() {
            this.on("update:Database", this.refresh, this);
            this.on("update:PreLoadPath", this.refresh, this);
        },
        refresh: function () {
            if (this.RootItem.indexOf("," + this.database + ",") === -1) {
                var parts = this.RootItem.split("|");
                var newParth = [];
                for (var n = 0; n < parts.length; n++) {
                    var uri = parts[n].split(",");
                    uri[1] = this.database;
                    newParth.push(uri.join(","));
                }
                this.RootItem = newParth.join("|");
                var childNodes = this.widget.apply(this.$el, ["getRoot"]).childList;
        ...
        render: function () {
            ...
            //    if (this.widget) {
            //        this.widget.apply(this.$el, ["getRoot"]).tree.options.idPrefix += this.id + "-";
        ...
        nodeExpanding: function (node) {
            ...
            var database = new _sc.Definitions.Data.Database(itemUri.databaseUri);
        ...
        appendLoadedChildren: function (parentNode, childrenNodes, destArray) {
            ...
            parentNode.tree.options.idPrefix += this.id + "-";
        ...
        loadKeyPath: function () {
            ...
            var currentNodeId = pathParts.shift();
            if (!currentNodeId) {
                return false;
            }
