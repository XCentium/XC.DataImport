...
{
    public class BatchNonSitecoreImportManager : XC.DataImport.Repositories.Migration.IBatchSitecoreImportManager
    {
        ...
        private void ProcessMappingFiles(string id, Action<string, string> statusMethod, string statusFilename, Item currentItem)
        {
            for (var i = 0; i < Mapping.Files.Count(); i++)
            {
                ...
                if (File.Exists(filePath))
                {
                    ...
                    {
                        SourceRepository = new NonSitecoreDatabaseRepository(mappingObject.Databases.Source, mappingObject, true, this.IncrementalUpdate),
                        TargetRepository = new NonSitecoreDatabaseRepository(mappingObject.Databases.Target, mappingObject, true, this.IncrementalUpdate),
            ...
            try
            {
                ...
                for (var i = 0; i < Mapping.Files.Count(); i++)
                {
                    ...
                    if (File.Exists(filePath))
                    {
                        ...
                        var individualManager = new NonSitecoreImportManager()
                        {
                            SourceRepository = new NonSitecoreDatabaseRepository(mappingObject.Databases.Source, mappingObject, false, this.IncrementalUpdate),
                            TargetRepository = new NonSitecoreDatabaseRepository(mappingObject.Databases.Target, mappingObject, false, this.IncrementalUpdate),
                            Mapping = mappingObject,
                            CurrentItem = CurrentItem,
                        };
        ...
        {
            JobOptions options = new JobOptions(id, "BatchNonSitecoreImportManager", Context.Site.Name, this, "Run", new object[] { id, statusMethod, statusFileName })
            {
